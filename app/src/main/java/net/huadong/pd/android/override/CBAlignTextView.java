 ...
 * 打乱原生的TextView的排版换行选择复制等问题。为了能够使右端尽可能的对齐，将右侧多出的空隙 尽可能的分配到该行的标点后面。达到两段对齐的效果。
 * </p>
 *
 */
public class CBAlignTextView extends TextView {
		...
		if (id == android.R.id.copy) {
			...
			if (isFocused()) {
				...
				try {
					...
					Method getSelectTextMethod = cls.getDeclaredMethod(
							"getTransformedText", new Class[] { int.class,
									int.class });
					getSelectTextMethod.setAccessible(true);
					CharSequence selectedText = (CharSequence) getSelectTextMethod
							.invoke(this, min, max);
					Method closeMenuMethod = cls
	...
	private String processText(Paint paint, String text, int width) {
		...
		for (String line : lines) {
			newText.append('\n');
			newText.append(processLine(paint, line, width - getPaddingLeft()
					- getPaddingRight()));
		}
	...
	private String processLine(Paint paint, String text, int width) {
		...
		for (int i = maxChineseCount; i < old.length(); i++) {
			if (paint.measureText(old.substring(startPosition, i + 1)) > width
					- spaceWidth) {
				// 右侧多余空隙宽度
				float gap = (width - spaceWidth - paint.measureText(old
						.substring(startPosition, i)));
	...
	public void onWindowFocusChanged(boolean hasWindowFocus) {
		if (!inProcess && oldText != null && oldText.length() != 0) {
			...
			setPadding(getPaddingLeft() + spaceWidth, getPaddingTop(),
